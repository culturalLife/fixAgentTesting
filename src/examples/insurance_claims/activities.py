...
def _get_telemetry_tracer():
    from telemetry import setup_telemetry_client
    _, tracer = setup_telemetry_client(service_name="insurance_claims_activities")
    return tracer
...
def _confined_local_path(raw_path: str) -> Path:
    ...
    ``photo_uri`` comes from workflow input, which is untrusted. Reading an
    arbitrary path (e.g. ``/etc/passwd`` or ``file:///…``) and shipping its
    bytes to the vision API would let a caller exfiltrate host files. We
    therefore resolve the path under the project working directory and reject
...
def _resolve_image_uri(photo_uri: str) -> str:
    ...
    HTTP(S) URLs and existing data URIs are returned as-is. ``file://`` URIs and
    plain paths are read from disk (confined to the project directory, see
    :func:`_confined_local_path`) and encoded as ``data:image/<ext>;base64,...``
    so Pixtral can consume them without network access to the filesystem.
    """
    if photo_uri.startswith(("http://", "https://", "data:")):
        return photo_uri

    if photo_uri.startswith("file://"):
        raw_path = url2pathname(unquote(photo_uri[7:]))
    else:
        raw_path = photo_uri
...
async def analyze_photo(photo_uri: str) -> PhotoAnalysisResult:
    ...
    tracer = _get_telemetry_tracer()
    execution_id = get_current_execution_id()

    with tracer.start_as_current_span("analyze_photo_activity_span") as activity_span:
        activity_span.set_attribute("gen_ai.workflow.name", "insurance-claims-triage")
        activity_span.set_attribute("gen_ai.workflow.execution_id", execution_id)
        activity_span.set_attribute("gen_ai.activity.name", "analyze_photo")
        activity_span.set_attribute("gen_ai.agent.name", "photo_analyst_agent")
        activity_span.set_attribute("photo.uri", photo_uri)
        ...
        try:
            ...
            activity_span.set_attribute("gen_ai.activity.result", result.model_dump_json())
            return result
        except Exception as exc:
            record_span_exception(activity_span, exc)
...
async def check_consistency(
    ...
    with tracer.start_as_current_span("check_consistency_activity_span") as activity_span:
        activity_span.set_attribute("gen_ai.workflow.name", "insurance-claims-triage")
        activity_span.set_attribute("gen_ai.workflow.execution_id", execution_id)
        activity_span.set_attribute("gen_ai.activity.name", "check_consistency")
        activity_span.set_attribute("gen_ai.agent.name", "consistency_checker_agent")
        ...
        try:
            ...
            activity_span.set_attribute("gen_ai.activity.status", "SUCCESS")
            activity_span.set_attribute("gen_ai.activity.result", json.dumps(res_dict))
            return res_dict
        except Exception as exc:
            record_span_exception(activity_span, exc)
            activity_span.set_attribute("gen_ai.activity.status", "FAILED")
...
async def score_fraud_risk(
    ...
    with tracer.start_as_current_span("score_fraud_risk_activity_span") as activity_span:
        activity_span.set_attribute("gen_ai.workflow.name", "insurance-claims-triage")
        activity_span.set_attribute("gen_ai.workflow.execution_id", execution_id)
        activity_span.set_attribute("gen_ai.activity.name", "score_fraud_risk")
        activity_span.set_attribute("gen_ai.agent.name", "fraud_evaluator_agent")
...
async def generate_triage_report(
    ...
    with tracer.start_as_current_span("generate_triage_report_activity_span") as activity_span:
        activity_span.set_attribute("gen_ai.workflow.name", "insurance-claims-triage")
        activity_span.set_attribute("gen_ai.workflow.execution_id", execution_id)
        activity_span.set_attribute("gen_ai.activity.name", "generate_triage_report")
        activity_span.set_attribute("gen_ai.agent.name", "report_generator_agent")
